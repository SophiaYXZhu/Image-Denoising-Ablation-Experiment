...
def save_state(save_filename, optimizer, scheduler, ema):
...
def resume_state(load_path, optimizer, scheduler, ema):
...
def generate_mask(img, width=4, mask_type='random'):
    ...
    if 'square' in mask_type:
        ...
        rd_idx = index.repeat(n * h // width * w // width).to(img.device) # 16384 = 128**2 个 0
        ...
        rd_pair_idx += torch.arange(start=0,
                                    ...
                                    device=img.device)
        mask[rd_pair_idx] = 1
        mask = mask.type_as(img).view(n, c//(width**2), h, w)
        for b in range(len(mask)):
            for c in range(len(mask[b])):
                mask[b][c] = torch.rot90(mask[b][c], 3, [0, 1])
        ...
        mask[rd_pair_idx] = 1
        mask = mask.type_as(img).view(n, c//(width**2), h, w)
    elif 'zigzag' in mask_type:
        index = int(mask_type.split('_')[-1])
        mask = torch.zeros([n, c//(width**2), h, w]).type_as(img)

        # x = 2*(width**2-1)
        # for b in range(len(mask)):
        #     for c in range(len(mask[b])):
        #         for w in range(0, len(mask[b][c]), x):
        #             for h in range(index, len(mask[b][c][w]), x):
        #                 for i in range(x):
        #                     if w+i < len(mask[b][c]) and h+i < len(mask[b][c][w]):
        #                         mask[b][c][w+i][h+i] = 1
        #                 for i in range(x-1):
        #                     if w+(x-i)-1 < len(mask[b][c]) and h+i+1 < len(mask[b][c][w]):
        #                         mask[b][c][w+(x-i)-1][h+i+1] = 1
        # for b in range(len(mask)):
        #     for c in range(len(mask[b])):
        #         end = mask[b][c][:, (x):(x+index)]
        #         mask[b][c][:, :index] = end

        for b in range(len(mask)):
            for c in range(len(mask[b])):
                for w in range(0, len(mask[b][c])-2*width**2, 2*width**2):
                    for h in range(index, len(mask[b][c][w])-2*width**2, 2*width**2):
                        for i in range(2*width**2):
                            mask[b][c][w+i][h+i] = 1
                        for i in range(2*width**2):
                            mask[b][c][w+2*width**2-i-1][h+i] = 1
        for b in range(len(mask)):
            for c in range(len(mask[b])):
                end = mask[b][c][:, (2*width**2):(2*width**2+index)]
                mask[b][c][:, :index] = end
    return mask
...
class Masker(object):
    def __init__(self, width=4, mode='interpolate', mask_type='square'):
        self.width = width
        self.mode = mode
    ...
    def train(self, img):
        ...
        for i in range(self.width**2):
            x, mask = self.mask(img, mask_type=self.mask_type+'_{}'.format(i))
            tensors[:,i,...] = x
            masks[:,i,...] = mask
...
if __name__ == '__main__':
    ...
    # Masker
    masker_1 = Masker(width=2, mode='interpolate', mask_type='square')
    masker_2 = Masker(width=2, mode='interpolate', mask_type='zigzag')

    # Network
    network_1 = UNet(in_channels=opt.n_channel,
                    out_channels=opt.n_channel,
                    wf=opt.n_feature)
    network_2 = UNet(in_channels=opt.n_channel,
                    out_channels=opt.n_channel,
                    wf=opt.n_feature)
    if opt.parallel:
        network_1 = torch.nn.DataParallel(network_1)
        network_2 = torch.nn.DataParallel(network_2)
    network_1 = network_1.cuda()
    network_2 = network_2.cuda()

    # about training scheme
    num_epoch = opt.n_epoch
    ratio = num_epoch / 100
    optimizer_1 = optim.Adam(network_1.parameters(), lr=opt.lr, weight_decay=opt.w_decay)
    optimizer_2 = optim.Adam(network_2.parameters(), lr=opt.lr, weight_decay=opt.w_decay)
    ema_1 = ExponentialMovingAverage(network_1.parameters(), decay=opt.w_decay)
    ema_2 = ExponentialMovingAverage(network_2.parameters(), decay=opt.w_decay)
    scheduler_1 = lr_scheduler.MultiStepLR(optimizer_1,
                                        ...
                                        gamma=opt.gamma)
    scheduler_2 = lr_scheduler.MultiStepLR(optimizer_2,
                                        milestones=[
                                            int(20 * ratio) - 1,
                                            int(40 * ratio) - 1,
                                            int(60 * ratio) - 1,
    ...
    if opt.resume is not None:
        epoch_init, optimizer_1, scheduler_1, ema_1 = resume_state(opt.resume, optimizer_1, scheduler_1, ema_1)
        epoch_init, optimizer_2, scheduler_2, ema_2 = resume_state(opt.resume, optimizer_2, scheduler_2, ema_2)
    if opt.checkpoint is not None:
        network_1 = load_network(opt.checkpoint, network_1, strict=True)
        network_2 = load_network(opt.checkpoint, network_2, strict=True)
    ...
    for epoch in range(epoch_init, opt.n_epoch + 1):
        ...
        for param_group in optimizer_1.param_groups:
            current_lr_1 = param_group['lr']
        for param_group in optimizer_2.param_groups:
            current_lr_2 = param_group['lr']
        current_lr = (current_lr_1 + current_lr_2) / 2
        ...
        for iteration, noisy in enumerate(TrainingLoader):
            ...
            net_input_1, mask_1 = masker_1.train(noisy)
            net_input_2, mask_2 = masker_2.train(noisy)
            noisy_output_1 = network_1(net_input_1)
            noisy_output_2 = network_2(net_input_2)
            n, c, h, w = noisy.shape
            noisy_output_1 = (noisy_output_1*mask_1).view(n,-1,c,h,w).sum(dim=1)
            noisy_output_2 = (noisy_output_2*mask_2).view(n,-1,c,h,w).sum(dim=1)
            noisy_diff_1 = noisy_output_1 - noisy
            noisy_diff_2 = noisy_output_2 - noisy

            with torch.no_grad():
                exp_output_1 = network_1(noisy)
                exp_output_2 = network_2(noisy)
            exp_diff_1 = exp_output_1 - noisy
            exp_diff_2 = exp_output_2 - noisy
            ...
            revisible_1 = noisy_diff_1 + beta * exp_diff_1
            loss_reg_1 = alpha * torch.mean(noisy_diff_1**2)
            loss_rev_1 = torch.mean(revisible_1**2)
            loss_noisy_1 = loss_reg_1 + loss_rev_1

            revisible_2 = noisy_diff_2 + beta * exp_diff_2
            loss_reg_2 = alpha * torch.mean(noisy_diff_2**2)
            loss_rev_2 = torch.mean(revisible_2**2)
            loss_noisy_2 = loss_reg_2 + loss_rev_2

            n2n_diff = noisy_output_1 - noisy_output_2
            loss_n2n = torch.mean(n2n_diff**2)

            loss_all = 0.4*loss_noisy_1 + 0.4*loss_noisy_2 + 0.2*loss_n2n
            ...
            ema_1.update()
            ema_2.update()
            noisy_output = 0.5*noisy_output_1 + 0.5*noisy_output_2
            exp_output = 0.5*exp_output_1 + 0.5*exp_output_2
            noisy_output = depth_to_space(noisy_output, block_size=2) # 4, 1, C, C
            exp_output = depth_to_space(exp_output, block_size=2)
            pred = ((noisy_output + beta*exp_output) / (1 + beta))
            noisy = depth_to_space(noisy, 2)
            b, c, w, h = noisy.shape
            noisy = noisy.view(w, h, b)
            pred = pred.view(w, h, b)
            pred = pred.cuda().data.clamp(0, 1).numpy()
            psnr_mid = calculate_psnr(noisy.numpy().astype(np.float32), pred.astype(np.float32), 1.0)
            ssim_mid = calculate_ssim((noisy*255.0+0.5).numpy().astype(np.float32), (pred*255.0+0.5).astype(np.float32))
            logger.info('{:04d} {:05d} Loss_Noisy_1={:.6f}, Loss_Noisy_2={:.6f}, Loss_N2N={:.6f}, Loss_All={:.6f}, Time={:.4f}, PSNR={:.6f}, SSIM={:.6f}'
                .format(epoch, iteration, loss_noisy_1.item(), loss_noisy_2.item(), loss_n2n.item(), loss_all.item(), time.time() - st, psnr_mid, ssim_mid))
        ...
        if epoch % opt.n_snapshot == 0 or epoch == opt.n_epoch:
            ...
            network_2.eval()
            # save checkpoint
            save_network(network_1, epoch, "model_1")
            save_network(network_2, epoch, "model_2")
            save_state("{}_1.state".format(epoch), optimizer_1, scheduler_1, ema_1)
            save_state("{}_2.state".format(epoch), optimizer_2, scheduler_2, ema_2)
            # validation
            save_model_path = os.path.join(opt.save_model_path, opt.log_name, systime)
            ...
            for valid_name, valid_data in valid_dict.items():
                ...
                for idx in range(num_img):
                    for idy in range(num_block):
                        with ema_1.average_parameters() and ema_2.average_parameters(): # TODO, check correctness
                            ...
                            with torch.no_grad():
                                ...
                                net_input_1, mask_1 = masker_1.train(noisy_im)
                                net_input_2, mask_2 = masker_2.train(noisy_im)
                                noisy_output_1 = (network_1(net_input_1)*mask_1).view(n,-1,c,h,w).sum(dim=1)
                                noisy_output_2 = (network_2(net_input_2)*mask_2).view(n,-1,c,h,w).sum(dim=1)
                                exp_output_1 = network_1(noisy_im)
                                exp_output_2 = network_2(noisy_im)
                                # unpack raw data
                                pred_dn_1 = depth_to_space(noisy_output_1, block_size=2)
                                pred_dn_2 = depth_to_space(noisy_output_2, block_size=2)
                                pred_exp_1 = depth_to_space(exp_output_1, block_size=2)
                                pred_exp_2 = depth_to_space(exp_output_2, block_size=2)
                                pred_mid_1 = (pred_dn_1 + beta*pred_exp_1) / (1 + beta)
                                pred_mid_2 = (pred_dn_2 + beta*pred_exp_2) / (1 + beta)

                            pred_dn = 0.5*pred_dn_1 + 0.5*pred_dn_2
                            pred_exp = 0.5*pred_exp_1 + 0.5*pred_exp_2
                            pred_mid = 0.5*pred_mid_1 + 0.5*pred_mid_2
                            ...
                            # calculate psnr
                            psnr_dn = calculate_psnr(im.astype(np.float32), pred_dn.astype(np.float32), 1.0)
                            avg_psnr_dn.append(psnr_dn)
                            ssim_dn = calculate_ssim((im*255.0).astype(np.float32), (pred_dn*255.0).astype(np.float32))
                            avg_ssim_dn.append(ssim_dn)

                            psnr_exp = calculate_psnr(im.astype(np.float32), pred_exp.astype(np.float32), 1.0)
                            avg_psnr_exp.append(psnr_exp)
                            ssim_exp = calculate_ssim((im*255.0).astype(np.float32), (pred_exp*255.0).astype(np.float32))
                            avg_ssim_exp.append(ssim_exp)

                            psnr_mid = calculate_psnr(im.astype(np.float32), pred_mid.astype(np.float32), 1.0)
                            avg_psnr_mid.append(psnr_mid)
                            ssim_mid = calculate_ssim((im*255.0).astype(np.float32), (pred_mid*255.0).astype(np.float32))
                            avg_ssim_mid.append(ssim_mid)

                            logger.info(
                                '{:04d} Iteration={}, Num_Block={}, PSNR_exp={:.6f}, SSIM_exp={:.6f}, PSNR_dn={:.6f}, SSIM_dn={:.6f}, PSNR_mid={:.6f}, SSIM_mid={:.6f}'
                                    .format(epoch, idx, idy, psnr_exp, ssim_exp, psnr_dn, ssim_dn, psnr_mid, ssim_mid))

                            # visualization
                            save_path = os.path.join(save_dir, "{}_{:03d}-{:03d}-{:03d}_clean.png".format(valid_name, idx, idy, epoch))
                            Image.fromarray(origin255.squeeze()).save(save_path)
                            save_path = os.path.join(save_dir, "{}_{:03d}-{:03d}-{:03d}_noisy.png".format(valid_name, idx, idy, epoch))
                            Image.fromarray(noisy255.squeeze()).save(save_path)

                            save_path = os.path.join(save_dir, "{}_{:03d}-{:03d}-{:03d}_dn.png".format(valid_name, idx, idy, epoch))
                            Image.fromarray(pred255_dn.squeeze()).save(save_path)
                            save_path = os.path.join(save_dir, "{}_{:03d}-{:03d}-{:03d}_exp.png".format(valid_name, idx, idy, epoch))
                            Image.fromarray(pred255_exp.squeeze()).save(save_path)
                            save_path = os.path.join(save_dir, "{}_{:03d}-{:03d}-{:03d}_mid.png".format(valid_name, idx, idy, epoch))
                ...
                with open(log_path, "a") as f:
                    f.writelines("epoch:{}, PSNR_dn:{:.6f}, SSIM_dn:{:.6f}, PSNR_exp:{:.6f}, SSIM_exp:{:.6f}, PSNR_mid:{:.6f}, SSIM_mid:{:.6f}\n".format(
